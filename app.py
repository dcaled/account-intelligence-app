...
from llm_client import stream_brief
...
def fmt_revenue(v: float) -> str:
    """Format a dollar value as $XM or $XK for compact display."""
    if v >= 1_000_000:
...
def render_sidebar(df: pd.DataFrame) -> tuple[list, list, list, str]:
    ...
    return (
        selected_actions or all_actions,
        selected_segments or all_segments,
        selected_regions or all_regions,
        sort_option,
    )
...
# ── Filter & sort ─────────────────────────────────────────────────────────────
def apply_filters(
    df: pd.DataFrame, actions: list, segments: list, regions: list, sort_option: str
) -> pd.DataFrame:
    """Filter and sort the accounts DataFrame based on sidebar selections."""
    sort_map = {
...
def render_account_table(filtered: pd.DataFrame, total: int):
    """Render the sortable account table and return the Streamlit selection event."""
    st.title("Account Intelligence")
    st.caption(
        f"Showing {len(filtered):,} of {total:,} accounts — click any row to view details"
    )
    ...
    table = pd.DataFrame(
        {
            ...
            "Inactive (days)": filtered["days_since_last_sales_activity"]
            .astype(int)
            .values,
        }
    )
...
def render_scores(account: pd.Series) -> None:
    """Render the 6-metric header row: ARR, projected ARR, utilization, attention, risk, opportunity."""
    (
        col_arr,
        col_projected,
        col_utilization,
    ...
    col_projected.metric(
        ...
        uplift_delta,
        delta_color="normal"
        if (account["expansion_signal"] or account["contraction_signal"])
        else "off",
    ...
    opportunity_delta_color = (
        "off" if account["opportunity_label"] == "Stable" else "normal"
    )
    col_opportunity.metric(
        ...
        help="0–100. Signals: AI adoption (35%), projected ARR growth (25%), seat saturation (25%), license coverage (15%). Expansion Ready ≥ 70 · Growth Signal ≥ 40 · Stable < 40.",
    )


def render_risk_signals(account: pd.Series) -> None:
    """Render the risk signals card: renewal, inactivity, tickets, and conditional warnings."""
    with st.container(border=True):
        ...
        col_last_activity.metric(
            "Last Activity",
            f"{int(account['days_since_last_sales_activity'])}d ago",
            help="Days since the last recorded sales interaction. Longer gaps increase risk.",
        )
        col_tickets.metric(
            "Open Tickets",
            int(account["nr_support_tickets"]),
        ...
        if days_inactive >= 90:
            st.markdown(
                f":red[⚠ No sales contact in {days_inactive} days — account may feel neglected going into renewal.]"
            )
        elif days_inactive >= 45:
            st.markdown(
                f":orange[Last contact {days_inactive} days ago — worth scheduling a check-in soon.]"
            )

        if account["contraction_signal"]:
            st.markdown(
                f":red[↓ ARR projected to contract by {abs(account['arr_uplift'] - 1):.0%} — revenue loss risk at renewal.]"
            )

        if account["seat_utilization"] < 0.50:
            unused = int(account["unused_seats"])
            st.markdown(
                f":red[⚠ {account['seat_utilization']:.0%} seat utilization — {unused} licensed seats are unused.]"
            )


def render_opportunity_signals(account: pd.Series) -> None:
    """Render the opportunity signals card: AI adoption, license coverage, and conditional highlights."""
    with st.container(border=True):
        st.markdown("**Opportunity Signals**")

        col_ai, col_coverage = st.columns(2)
        col_ai.metric(
            "AI Adoption",
            f"{account['ai_usage']:.0%}",
        ...
        col_coverage.metric(
            "License Coverage",
            f"{account['license_coverage']:.0%}",
            help="Licensed seats ÷ total employees. Below 25%, most of the company isn't on the platform yet — meaningful expansion room exists. Contributes 15% to the opportunity score.",
        )

        if account["expansion_signal"]:
            st.markdown(
                f":green[↑ ARR projected to grow by {abs(account['arr_uplift'] - 1):.0%} — model already signals expansion.]"
            )

        if account["seat_utilization"] > 0.75:
            st.markdown(
                f":green[↑ {account['seat_utilization']:.0%} seat utilization — account is near capacity and ready for an expansion conversation.]"
            )


def render_account_detail(account: pd.Series) -> None:
    """Render the full detail panel for a selected account: scores, signals, last call, and LLM brief."""
    st.divider()
    ...
    system_prompt, user_message = build_prompt(account)
    ...
    if st.button("Generate Meeting Brief", type="primary", use_container_width=True):
        system_prompt, user_message = build_prompt(account)
        brief = st.write_stream(stream_brief(system_prompt, user_message))
        st.session_state[brief_key] = brief
    elif st.session_state.get(brief_key):
